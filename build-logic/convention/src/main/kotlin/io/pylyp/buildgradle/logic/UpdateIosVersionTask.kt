 ...
 *
 * This plugin:
 * - Reads app-version and app-build from Constants
...
class IosVersionUpdatePlugin : Plugin<Project> {
    override fun apply(project: Project) {
        project.afterEvaluate {
            ...
            val appBuild = Constants.APP_BUILD.toString()
            ...
            project.tasks.register<UpdateIosVersionTask>("updateIosVersion") {
                ...
                this.appBuild.set(appBuild)
                this.infoPlistFile.set(infoPlistFile)
                this.pbxprojFile.set(pbxprojFile)

                group = "versioning"
                description =
                    "Updates iOS version in Info.plist and project.pbxproj from Constants"
            }
        }
    }
}
 ...
 * Updates both Info.plist and project.pbxproj with version and build number from Constants.
 */
